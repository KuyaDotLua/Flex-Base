...
namespace ImTricks {
	...
	namespace Animations {
		...
		int FastIntLerp(const char* identifier, bool state, int min, int max, int speed) {
			...
			const float frameRateSpeed = speed * (1.f - FlexUI::GetIO().DeltaTime);
		...
		ImColor FastColorLerp(ImColor start, ImColor end, float stage)
		{
			...
			return FlexUI::ColorConvertFloat4ToU32(lerp);
		}
	...
	namespace NotifyManager {
		...
		void HandleNotifies(ImDrawList* draw) {
			...
			const auto ScreenSize = FlexUI::GetIO().DisplaySize;
			...
			auto DrawNotify = [&draw, &NotifyPos](NotifyStruct notify) {
				...
				draw->AddRectFilled(NotifyPos, NotifyEndPos, FlexUI::GetColorU32(FlexUICol_PopupBg), FlexUI::GetStyle().PopupRounding);
				...
				const auto TextSize = FlexUI::CalcTextSize(notify.message);
				const auto TextPos = NotifyPos + ImVec2(15.f, 15.f - TextSize.y / 2.f);

				draw->AddText(TextPos, FlexUI::GetColorU32(FlexUICol_Text), notify.message);
	...
	namespace Widgets {
		void ColorEdit3(const char* label, ImColor& color, FlexUIColorEditFlags flags) {
			float col[3] = { color.Value.x, color.Value.y, color.Value.z };

			if (FlexUI::ColorEdit3(label, col, flags))
				color = ImColor(col[0], col[1], col[2]);
		}

		void ColorEdit4(const char* label, ImColor& color, FlexUIColorEditFlags flags) {
			float col[4] = { color.Value.x, color.Value.y, color.Value.z , color.Value.w };

			if (FlexUI::ColorEdit4(label, col, flags))
				color = ImColor(col[0], col[1], col[2], col[3]);
		}

		void ColorEdit3(const char* label, ImTrickyColor& color, FlexUIColorEditFlags flags) {
		...
		void ColorEdit4(const char* label, ImTrickyColor& color, FlexUIColorEditFlags flags) {
